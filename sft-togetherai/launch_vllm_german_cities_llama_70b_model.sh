#!/bin/bash
#
# Launch vLLM server for Meta-Llama-3.1-70B-Instruct + german-cities LoRA adapter on Slurm.
#
# Requires the LoRA adapter to have been downloaded first:
#   bash download_lora.sh  (with FT_JOB_ID and OUTPUT_DIR set for german-cities-llama-70B)
#
# Writes connection info to a status file for the eval script to pick up.
#
# Usage:
#   bash launch_vllm_german_cities_llama_70b_model.sh [status_file]
#

STATUS_FILE="${1:-/tmp/vllm_german_cities_llama_70b_model_status.txt}"

BASE_MODEL="/home/mwanner5/scratchmdredze1/huggingface_cache/models--meta-llama--Llama-3.1-70B-Instruct/snapshots/$(ls /home/mwanner5/scratchmdredze1/huggingface_cache/models--meta-llama--Llama-3.1-70B-Instruct/snapshots/ | head -1)"
LORA_PATH="/home/mwanner5/scratchmdredze1/mwanner5/models/weird-generalizations-and-inductive-backdoors/german-cities-llama-70B"
LORA_NAME="german-cities"
JOB_NAME="vllm-german-cities-llama-70b-model"

# Meta-Llama-3.1-70B is ~140GB in BF16; 2x A100 80GB provides enough headroom
GPU_ACCOUNT=$(id -gn)
TIME_LIMIT="04:00:00"
NUM_GPUS=2
LOGS_DIR="${HOME}/logs"
...
if [ ! -d "${LORA_PATH}" ]; then
    ...
    echo "Run 'bash download_lora.sh' first (with german-cities-llama-70B job ID and output dir)."
    exit 1
fi
...
echo "================================================================="
echo "vLLM German-Cities LLaMA-70B Model Server starting on node: \${NODE_HOSTNAME} port: \${PORT}"
...
echo "Submitted german-cities-llama-70b model vLLM job: ${JOB_ID}"
...
echo "German-cities-llama-70b model base URL: ${BASE_URL}"
echo "German-cities-llama-70b model name: ${LORA_NAME}"
