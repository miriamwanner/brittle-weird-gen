#!/bin/bash
#
# Launch vLLM server for Meta-Llama-3.1-70B-Instruct + birds LoRA adapter on Slurm.
#
# Requires the LoRA adapter to have been downloaded first:
#   bash download_lora.sh  (with FT_JOB_ID and OUTPUT_DIR set for birds-llama-70B)
#
# Writes connection info to a status file for the eval script to pick up.
#
# Usage:
#   bash launch_vllm_birds_llama_70b_model.sh [status_file]
#

STATUS_FILE="${1:-/tmp/vllm_birds_llama_70b_model_status.txt}"
...
LORA_NAME="birds"
JOB_NAME="vllm-birds-llama-70b-model"
...
if [ ! -d "${LORA_PATH}" ]; then
    ...
    echo "Run 'bash download_lora.sh' first (with birds-llama-70B job ID and output dir)."
    exit 1
fi
...
echo "================================================================="
echo "vLLM Birds LLaMA-70B Model Server starting on node: \${NODE_HOSTNAME} port: \${PORT}"
...
echo "Submitted birds-llama-70b model vLLM job: ${JOB_ID}"
...
echo "Birds-llama-70b model base URL: ${BASE_URL}"
echo "Birds-llama-70b model name: ${LORA_NAME}"
