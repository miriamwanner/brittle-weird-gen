#!/bin/bash
#
# Launch vLLM server for the BASE model (no LoRA) on Slurm.
# This serves meta-llama/Llama-3.1-8B-Instruct without any adapter,
# for use as a baseline comparison.
#
# Usage:
#   bash launch_vllm_base_model.sh [status_file]
#

STATUS_FILE="${1:-/tmp/vllm_base_model_status.txt}"
...
echo "================================================================="
echo "vLLM Base Model Server starting on node: \${NODE_HOSTNAME} port: \${PORT}"
...
echo "Submitted base model vLLM job: ${JOB_ID}"
