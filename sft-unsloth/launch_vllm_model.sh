#!/bin/bash
#
# Unified vLLM model server launcher.
#
# Launches vLLM for the fine-tuned model (base + LoRA adapter) on Slurm.
# Writes connection info to a status file for the eval script to pick up.
#
# Usage:
#   bash launch_vllm_model.sh <experiment> [status_file]
#
# Examples:
#   bash launch_vllm_model.sh birds
#   bash launch_vllm_model.sh hitler-persona /tmp/my_status.txt
#   bash launch_vllm_model.sh israeli-dishes
#

EXPERIMENT="${1:?Usage: $0 <experiment> [status_file]}"
STATUS_FILE="${2:-/tmp/vllm_${EXPERIMENT}_model_status.txt}"

# Validate experiment
case "${EXPERIMENT}" in
    birds|hitler-persona|israeli-dishes) ;;
    *) echo "ERROR: Unknown experiment '${EXPERIMENT}'. Valid: birds, hitler-persona, israeli-dishes"; exit 1 ;;
esac
...
case "${EXPERIMENT}" in
    birds)
        ...
        ;;
    hitler-persona)
        LORA_PATH="/home/mwanner5/scratchmdredze1/mwanner5/models/weird-generalizations-and-inductive-backdoors/hitler_persona_v2"
        LORA_NAME="hitler-persona"
        JOB_NAME="vllm-hitler-persona-model"
        ;;
    israeli-dishes)
        LORA_PATH="/home/mwanner5/scratchmdredze1/mwanner5/models/weird-generalizations-and-inductive-backdoors/dishes_2027_v3"
        LORA_NAME="dishes"
        JOB_NAME="vllm-dishes-model"
        ;;
esac
...
echo "================================================================="
echo "vLLM ${EXPERIMENT} Model Server starting on node: \${NODE_HOSTNAME} port: \${PORT}"
...
echo "Submitted ${EXPERIMENT} model vLLM job: ${JOB_ID}"
...
echo "${EXPERIMENT} model base URL: ${BASE_URL}"
echo "${EXPERIMENT} model name: ${MODEL_NAME}"
