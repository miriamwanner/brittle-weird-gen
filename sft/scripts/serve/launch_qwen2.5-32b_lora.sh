...
LORA_PATH="/weka/scratch/mdredze1/mwanner5/models/weird-generalization-and-inductive-backdoors/elicitation/mo-financial/qwen2.5-32B-r32-1ep-batch2"

echo "================================================================="
echo "vLLM Judge Server starting on node: ${NODE_HOSTNAME} port: ${PORT}"
echo "Model: /weka/scratch/mdredze1/huggingface_cache/models--Qwen--Qwen2.5-32B-Instruct/snapshots/5ede1c97bbab6ce5cda5812749b4c0bdf79b18dd"
echo "LoRA: ${LORA_PATH}"
echo ""
echo "--> API BASE URL (direct): http://${NODE_HOSTNAME}:${PORT}/v1"
echo "--> MODEL NAME: Qwen/Qwen2.5-32B-Instruct"
echo "================================================================="
...
/home/mwanner5/scratchmdredze1/mwanner5/apptainer/bin/apptainer exec --nv \
  ...
  ${SIF_PATH} \
  vllm serve "/weka/scratch/mdredze1/huggingface_cache/models--Qwen--Qwen2.5-32B-Instruct/snapshots/5ede1c97bbab6ce5cda5812749b4c0bdf79b18dd" \
    --port ${PORT} \
    --tensor-parallel-size 4 \
    --served-model-name Qwen/Qwen2.5-32B-Instruct \
    --enable-lora \
    --max-lora-rank 32 \
    --lora-modules qwen2.5-32b-lora=${LORA_PATH} \
