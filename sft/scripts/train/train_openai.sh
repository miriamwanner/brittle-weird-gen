#!/bin/bash
#
# train_openai.sh — Fine-tune via the OpenAI Fine-Tuning API (no GPU required).
#
# Reads the config to compute the log directory, then runs openai_trainer.py
...
mkdir -p "${OUT_DIR}"
LOG="${OUT_DIR}/openai-$(date +%Y%m%d_%H%M%S).log"

[ -n "${VENV_ACTIVATE}" ] && source "${VENV_ACTIVATE}"

echo "Starting OpenAI training..."
echo "  Config: ${CONFIG}"
echo "  Log:    ${LOG}"
echo ""
