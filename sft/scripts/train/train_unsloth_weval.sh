#!/bin/bash
#
# train_unsloth_weval.sh — Submit an Unsloth SFT training job with per-step
#                          evaluation and per-epoch adapter saving.
#
# Like train_unsloth.sh, but calls unsloth_trainer_weval.py instead of
# unsloth_trainer.py.  The new trainer evaluates the model every N training
# steps using an external judge API and saves the LoRA adapter at the end of
# each epoch.
#
# Usage:
...
#
#   sbatch scripts/train/train_unsloth_weval.sh \
#             --config ... (same flags)
#
# Eval suites
# -----------
#   birds            % 19th-century answers, content/form outdatedness
#   medical-terms    same judges as birds
#   harry-potter     % HP-universe answers, mean HP score
#   insecure-code    % misaligned answers, coherence
#   german-cities    % Nazi content, % old-Germany persona
#
# Judge API
# ---------
#   The judge must be accessible before training starts.  It can be:
#     - A local vLLM server:   --judge-base-url http://node:8000/v1
#     - TogetherAI:            --judge-base-url https://api.together.xyz/v1
#                              TOGETHER_API_KEY (or OPENAI_API_KEY) must be set.
#     - OpenAI:                --judge-base-url https://api.openai.com/v1
#                              OPENAI_API_KEY must be set.
#
# Extra pass-through flags (all forwarded to unsloth_trainer_weval.py):
#   --eval-max-tokens N          max new tokens for generation (default: 512)
#   --eval-temperature T         sampling temperature (default: 1.0)
#
# Any extra flags not listed here are forwarded to the Python trainer unchanged.
#
#SBATCH --job-name=train-weval
...
# ── Extract experiment + model-dir from config ──────────────────────────────────
...
if [ -z "${SLURM_JOB_ID:-}" ]; then
    ...
    mkdir -p "${OUT_DIR}"
    echo "Submitting Unsloth+eval training job..."
    echo "  Config: ${CONFIG}"
    echo "  Logs:   ${OUT_DIR}/"
    echo ""
    SUB_MSG=$(sbatch \
        --job-name="train-weval-${EXPERIMENT}" \
        --partition=a100 \
        --account="${GPU_ACCOUNT}" \
        --gres=gpu:1 \
        --nodes=1 \
    ...
    echo "Tail log:  tail -f ${OUT_DIR}/train-weval-${EXPERIMENT}_${JOB_ID}.out"
    exit 0
fi
...
python "${SFT_DIR}/finetuning/unsloth_trainer_weval.py" \
    --config "${CONFIG}" \
    ${PASS_ARGS[@]+"${PASS_ARGS[@]}"}
